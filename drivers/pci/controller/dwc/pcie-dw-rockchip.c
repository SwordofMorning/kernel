					 ...
					 struct rk_pcie *rk_pcie)
{
	...
	rk_pcie->en_gpio = devm_gpiod_get_optional(&pdev->dev, "enable",
                                                    GPIOD_OUT_HIGH);
        if (IS_ERR(rk_pcie->en_gpio)) {
                dev_err(&pdev->dev, "invalid enable-gpios property in node\n");
                return PTR_ERR(rk_pcie->en_gpio);
        }
	gpiod_set_value(rk_pcie->en_gpio, 1);
